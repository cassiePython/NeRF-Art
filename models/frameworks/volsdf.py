...
def error_bound(d_vals, sdf, alpha, beta):
    ...
    # [..., N_pts-1]
    d_i_star = torch.clamp_min(
        0.5 * (sdf_abs_i[..., :-1] + sdf_abs_i[..., 1:] - delta_i), 0.)
    # [..., N_pts-1]
...
def fine_sample(implicit_surface_fn, init_dvals, rays_o, rays_d,
    ...
    with torch.no_grad():
        ...
        def opacity_invert_cdf_sample(d_vals_, sdf_, alpha_, beta_, N_importance=final_N_importance, det=not perturb):
            ...
            fine_dvals = rend_util.sample_cdf(
                d_vals_, opacity_approx, N_importance, det=det)
            return fine_dvals

        # final output storage.
        # being updated during the iterations of the algorithm
        final_fine_dvals = torch.zeros(
            [*prefix, final_N_importance]).to(device)
        ...
        beta = torch.sqrt(
            (far**2) / (4 * (init_dvals.shape[-1]-1) * np.log(1+eps)))
        alpha = 1./beta
        # alpha = alpha_net
        # [*prefix, N]
        ...
        if use_view_dirs:
            ...
            # when not use view_dir, the forward will return geometry detail
            sdf, _ = query_sdf(d_vals, rays_o, rays_d)
        net_bounds_max = error_bound(
            d_vals, sdf, alpha_net, beta_net).max(dim=-1).values
        ...
        final_converge_flag = torch.zeros(
            [*prefix], device=device, dtype=torch.bool)

        # NOTE: these are the final fine sampling points for those rays that satisfy eps condition at the very beginning.
        if (~mask).sum() > 0:
            final_fine_dvals[~mask] = opacity_invert_cdf_sample(
                d_vals[~mask], sdf[~mask], alpha_net, beta_net)
        ...
        while it_algo < max_iter:
            ...
            if mask.sum() > 0:
                ...
                upsampled_d_vals_masked = rend_util.sample_pdf(
                    d_vals[mask], bounds_masked, N_up+2, det=True)[..., 1:-1]
                ...
                d_vals = torch.cat([d_vals, torch.zeros(
                    [*prefix, N_up]).to(device)], dim=-1)
                # TODO Aug 06: check
                if isinstance(sdf, tuple):
                    sdf = sdf[0]
                sdf = torch.cat(
                    [sdf, torch.zeros([*prefix, N_up]).to(device)], dim=-1)
                ...
                d_vals_masked, sort_indices_masked = torch.sort(
                    d_vals_masked, dim=-1)
                if not require_nablas:
                    sdf_masked[..., cur_N:cur_N+N_up], _ = query_sdf(
                        upsampled_d_vals_masked, rays_o[mask], rays_d[mask])
                else:
                    sdf_masked[..., cur_N:cur_N+N_up], _, __ = query_sdf(
                        upsampled_d_vals_masked, rays_o[mask], rays_d[mask])

                sdf_masked = torch.gather(
                    sdf_masked, dim=-1, index=sort_indices_masked)
                ...
                net_bounds_max[mask] = error_bound(
                    d_vals[mask], sdf[mask], alpha_net, beta_net).max(dim=-1).values
                ...
                if converged_mask.sum() > 0:
                    ...
                    final_fine_dvals[converged_mask] = opacity_invert_cdf_sample(
                        d_vals[converged_mask], sdf[converged_mask], alpha_net, beta_net)
                ...
                if (sub_mask_of_mask).sum() > 0:
                    ...
                    beta_left = beta_net * \
                        torch.ones_like(beta_right, device=device)
                    ...
                    for _ in range(max_bisection):
                        ...
                        # alpha_tmp = alpha_net
                        # [Submasked]
                        bounds_tmp_max = error_bound(
                            d_vals_tmp, sdf_tmp, alpha_tmp, beta_tmp).max(dim=-1).values
                        beta_right[bounds_tmp_max <=
                                   eps] = beta_tmp[bounds_tmp_max <= eps]
                        beta_left[bounds_tmp_max >
                                  eps] = beta_tmp[bounds_tmp_max > eps]
                    ...
                    # ----------------
                    bounds_masked = error_bound(
                        d_vals_tmp, sdf_tmp, alpha[new_mask], beta[new_mask])
                    # bounds_masked = error_bound(d_vals_tmp, rays_d_tmp, sdf_tmp, alpha_net, beta[new_mask])
                    # NOTE: prevent INF caused NANs
        ...
        if (~final_converge_flag).sum() > 0:
            ...
            final_fine_dvals[~final_converge_flag] = opacity_invert_cdf_sample(
                d_vals[~final_converge_flag], sdf[~final_converge_flag], alpha_plus, beta_plus)
...
class VolSDF(nn.Module):
    def __init__(self,
        ...
        self.ln_beta = nn.Parameter(data=torch.Tensor(
            [ln_beta_init]), requires_grad=True)
    ...
    def forward_surface_with_nablas(self, x: torch.Tensor):
        ...
        if self.use_sphere_bg:
            ...
            # NOTE: in case the normals changed suddenly near the sphere.
            outside_sphere = d_bg < sdf
    ...
    def forward(self, x: torch. Tensor, view_dirs: torch.Tensor = None, return_nablas=False):
        ...
        if not return_nablas and view_dirs is None:
            ...
            radiances = self.radiance_net.forward(
                x, view_dirs, None, geometry_feature)
...
def volume_render(
    ...
    **dummy_kwargs  # just place holder
    ...
    def render_rayschunk(rays_o: torch.Tensor, rays_d: torch.Tensor, return_nablas: bool):
        ...
        if use_nerfplusplus:
            _, fars, mask_intersect = rend_util.get_sphere_intersection(
        ...
        with torch.no_grad():
            ...
            # NOTE: you might want to use less samples for faster training.
        ...
        if use_view_dirs:
            radiances, sdf, nablas = batchify_query(
                model.forward, pts, view_dirs.unsqueeze(-2).expand_as(pts), return_nablas=require_nablas)
        else:
            radiances, sdf, nablas = batchify_query(
                model.forward, pts, return_nablas=return_nablas)
        # [(B), N_rays, N_pts]
        sigma = sdf_to_sigma(sdf, alpha, beta)
        ...
        if use_nerfplusplus:
            _t = torch.linspace(
                0, 1, N_outside + 2)[..., 1:-1].float().to(device)
            ...
            pts_out = rays_o[..., None, :] + \
                rays_d[..., None, :] * d_out[..., :, None]
            x_out = torch.cat(
                [pts_out/rs[..., None], 1./rs[..., None]], dim=-1)
            sigma_out, radiance_out = batchify_query(
                model.nerf_outside.forward, x_out, view_dirs.unsqueeze(-2).expand_as(pts_out) if use_view_dirs else None)
        ...
        # [(B), N_rays, 1]
        depth_map = torch.sum(
            tau_i / (tau_i.sum(-1, keepdim=True)+1e-10) * d_all[..., :-1], dim=-1)
...
def get_model(args, render_target):
    ...
    render_kwargs_train = {
        ...
        'perturb': args.model.setdefault('perturb', True),
    ...
    trainer = Trainer(model, args.device_ids,
                      batched=render_kwargs_train['batched'], is_finetune=args.training.is_finetune, target_hw=render_target)
