...
class NeuS(nn.Module):
    def __init__(self,
        ...
        self.ln_s = nn.Parameter(data=torch.Tensor(
    ...
    def render_rayschunk(rays_o: torch.Tensor, rays_d: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            if upsample_algo == 'direct_use':   # nerf-like
                ...
                pts_coarse = rays_o.unsqueeze(-2) + \
                    d_coarse.unsqueeze(-1) * rays_d.unsqueeze(-2)
                ...
                *_, w_coarse = sdf_to_w(sdf_coarse, 1./fixed_s_recp)
                ...
                d_fine = rend_util.sample_pdf(
                    d_coarse, w_coarse, N_importance, det=not perturb)
                # Gather points
            ...
            # used config: N_nograd_samples
                ...
                _d = near * (1 - _t) + far * _t
                _pts = rays_o.unsqueeze(-2) + \
                    _d.unsqueeze(-1) * rays_d.unsqueeze(-2)
                ...
                *_, _w = sdf_to_w(_sdf, 1./fixed_s_recp)
                d_fine = rend_util.sample_pdf(
                    _d, _w, N_importance, det=not perturb)
                # Gather points
                d_all = torch.cat([d_coarse, d_fine], dim=-1)
                d_all, d_sort_indices = torch.sort(d_all, dim=-1)
            ...
            # used config: N_upsample_iters
                ...
                for i in range(N_upsample_iters):
                    ...
                    prev_dot_val = torch.cat(
                        [torch.zeros_like(dot_val[..., :1], device=device), dot_val[..., :-1]], dim=-1)
                    ...
                    sdf_fine = batchify_query(model.implicit_surface.forward, rays_o.unsqueeze(
                        -2) + d_fine.unsqueeze(-1) * rays_d.unsqueeze(-2))
        ...
        sdf, nablas, _ = batchify_query(
            model.implicit_surface.forward_with_nablas, pts)
        ...
        radiances = batchify_query(model.forward_radiance, pts_mid, view_dirs.unsqueeze(
            -2).expand_as(pts_mid) if use_view_dirs else None)
        ...
        if N_outside > 0:
            ...
            d_vals_out = torch.cat([d_mid, d_vals_out],
                                   dim=-1)  # already sorted
            pts_out = rays_o[..., None, :] + \
                rays_d[..., None, :] * d_vals_out[..., :, None]
            r = pts_out.norm(dim=-1, keepdim=True)
            x_out = torch.cat([pts_out/r, 1./r], dim=-1)
            views_out = view_dirs.unsqueeze(-2).expand_as(
                x_out[..., :3]) if use_view_dirs else None

            #! Aug 14: dummy variable for volsdf return nabla compability
            sigma_out, radiance_out, _ = batchify_query(
            ...
            alpha_out = 1 - torch.exp(-F.softplus(sigma_out) * dists)
        ...
        if N_outside > 0:
            ...
            # [(B), N_ryas, N_pts-1]
            alpha_in = opacity_alpha * mask_inside.float() + \
                alpha_out[..., :N_pts_1] * (~mask_inside).float()
            # [(B), N_ryas, N_pts-1 + N_outside]
            opacity_alpha = torch.cat(
                [alpha_in, alpha_out[..., N_pts_1:]], dim=-1)

            # [(B), N_ryas, N_pts-1, 3]
            radiance_in = radiances * \
                mask_inside.float()[..., None] + radiance_out[...,
                                                              :N_pts_1, :] * (~mask_inside).float()[..., None]
            # [(B), N_ryas, N_pts-1 + N_outside, 3]
            radiances = torch.cat(
                [radiance_in, radiance_out[..., N_pts_1:, :]], dim=-2)
            d_final = d_vals_out
        else:
            d_final = d_mid
        ...
        depth_map = torch.sum(
            visibility_weights / (visibility_weights.sum(-1, keepdim=True)+1e-10) * d_final, -1)
        ...
        if calc_normal:
            ...
            normals_map = (
                normals_map[..., :N_pts, :] * visibility_weights[..., :N_pts, None]).sum(dim=-2)
