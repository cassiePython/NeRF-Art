...
def batchify_query(query_fn, *args: Iterable[torch.Tensor], chunk, dim_batchify,return_nablas):
    # [(B), N_rays, N_pts, ...] -> [(B), N_rays*N_pts, ...]
    _N_rays = args[0].shape[dim_batchify]
    _N_pts = args[0].shape[dim_batchify+1]
    #remove None element in args
    ...
    for i in range(0, _N, chunk):
        ...
        raw_ret_i = query_fn(*args_i,return_nablas = return_nablas)
    ...
    for entry in zip(*raw_ret):
        ...
        collect_raw_ret.append(entry)
        num_entry += 1
    if num_entry == 1:
        return collect_raw_ret[0]
    elif num_entry == 2:
        collect_raw_ret += [None] # placeholder for dummy nabla output
    return tuple(collect_raw_ret)